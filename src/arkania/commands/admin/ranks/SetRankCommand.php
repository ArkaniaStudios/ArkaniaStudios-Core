...
class SetRankCommand extends BaseCommand {
    ...
    public function execute(CommandSender $player, string $commandLabel, array $args): bool{
        ...
        if (!in_array($args[1], $vip_rank))
            Server::getInstance()->broadcastMessage(Utils::getPrefix() . "§c" . $target . "§f vient de recevoir le grade §c" . $this->core->ranksManager->getRankColorToString($args[1]) . " §f!");
        else
            Server::getInstance()->broadcastMessage(Utils::getPrefix() . "§c" . $target . "§f vient d'acheter le grade §c" . $this->core->ranksManager->getRankColorToString($args[1]) . "§f !");
